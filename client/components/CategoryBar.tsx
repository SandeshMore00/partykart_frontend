...
export default function CategoryBar() {
  ...
  // ✅ Hover only when no activeCategory (desktop only)
  const handleCategoryHover = (categoryId: number) => {
      ...
      <div className="container mx-auto px-2 sm:px-4">
        ...
        >
          {categories.map((category) => (
            ...
            >
              <button
                className={`flex items-center justify-center sm:justify-start space-x-1 sm:space-x-2 
        ...
        {(
          // Desktop: show when hovered OR when clicked
          (!isMobile &&
            ((hoveredCategory && (categoryHovered || dropdownHovered)) || activeCategory)) ||
          // Mobile: show only when clicked
          ...
          >
            <div className="py-2">
              <div className="px-3 py-2 text-sm font-semibold text-gray-500 border-b border-gray-100 flex items-center justify-between">
                {categories.find(
                  (c) => c.category_id === (activeCategory || hoveredCategory)
                )?.category_name}
                {activeCategory && isMobile && (
                  <button
                    className="text-xs text-pink-600 hover:underline ml-2"

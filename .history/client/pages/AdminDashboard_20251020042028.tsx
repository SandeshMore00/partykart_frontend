...
export default function AdminDashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Always fetch fresh categories data
      ...
      // Always fetch fresh subcategories data
  ...
  // Numeric validation for shipping fields
  ...
  const handleCreateProduct = async () => {
    ...
    try {
      ...
      // Append shipping fields if provided
      ...
      if (productForm.height) {
        formData.append('height', productForm.height);
      }
      if (productForm.origin_location) {
  ...
  const handleUpdateProduct = async () => {
    ...
    if (productForm.weight && productForm.weight !== '') {
      formData.append('weight', productForm.weight);
      hasChange = true;
    }
    if (productForm.length && productForm.length !== '') {
      formData.append('length', productForm.length);
      hasChange = true;
    }
    if (productForm.width && productForm.width !== '') {
      formData.append('width', productForm.width);
      hasChange = true;
    }
    if (productForm.height && productForm.height !== '') {
      formData.append('height', productForm.height);
      hasChange = true;
    }
    if (productForm.origin_location && productForm.origin_location !== '') {
      formData.append('origin_location', productForm.origin_location);
      hasChange = true;
    }
  ...
  file: null,
  // ensure shipping fields exist to satisfy the productForm shape
  ...
  origin_location: (product as any).origin_location ? String((product as any).origin_location) : ''
    });
    setShowProductModal(true);
  };
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[100]">
          <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4 max-h-[90vh] flex flex-col">
            ...
            <div className="space-y-4 overflow-y-auto flex-1 pr-2">
              ...
              {/* Subcategory moved below to avoid dropdown clipping in modal scroll area */}
              
              {/* Shipping Information Section */}
              <div className="border-t pt-4">
                <h4 className="text-sm font-medium text-gray-700 mb-3">Shipping Information (Optional)</h4>
            ...
            {/* Product Image moved out of scrollable area so Subcategory can be last */}
            <div>
            ...
            {/* Subcategory select placed outside scrollable area to avoid clipping */}
            <div>

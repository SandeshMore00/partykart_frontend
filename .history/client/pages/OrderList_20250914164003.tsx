...
export default function OrderList() {
  ...
  const fetchOrders = async () => {
    await fetchOrdersFromApi("http://localhost:9024/v1/order_alert/");
  };

  const fetchAllOrders = async () => {
    await fetchOrdersFromApi("http://localhost:9024/v1/order_alert/all-order");
  };

  const fetchOrdersFromApi = async (url: string) => {
    setLoading(true);
    setError("");
    try {
  ...
  const isAdmin =
    user?.role === "admin" || user?.role === "super_admin" || user?.role === 1 || user?.role === 2;
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center space-x-3">
          {isAdmin && (
            ...
            >
              <Package className="w-4 h-4" />
              <span>{loading ? "Loading..." : "Load All Orders"}</span>
            </Button>
          )}
          <Button
            onClick={fetchOrders}

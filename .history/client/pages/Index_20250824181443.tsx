          ...
          <div className="mt-6 flex flex-wrap justify-center gap-4">
            {categories.map((category) => (
              <div
                ...
                onMouseEnter={() => {
                  setBrowseHoveredCategory(category.category_id);
                  fetchBrowseSubcategories(category.category_id);
                }}
                onMouseLeave={() => {
                  setBrowseHoveredCategory(null);
                  setBrowseSubcategories([]);
                }}
                style={{ display: 'inline-block' }}
              >
                <Button className="px-6 py-3 bg-white text-pink-600 border border-pink-200 rounded-lg shadow hover:bg-pink-50">
                  {category.category_name}
                </Button>
                {browseHoveredCategory === category.category_id && browseSubcategories.length > 0 && (
                  <div
                    className="absolute top-0 left-full ml-2 w-56 bg-white border border-gray-200 rounded-lg shadow-xl z-50"
                    style={{ minHeight: '100%' }}
                  >
                    <div className="py-2">
                      ...
                      <div className="max-h-64 overflow-y-auto">
                        {browseSubcategories.map((subcategory) => (
                          <button
                            key={subcategory.sub_category_id}
                            onClick={() => handleSubcategoryClick(subcategory.sub_category_id)}

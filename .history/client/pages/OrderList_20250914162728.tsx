...
export default function OrderList() {
  ...
  // Default fetch: user-specific orders
  useEffect(() => {
    if (user) {
      fetchOrders();
    }
  ...
  const fetchAllOrders = async () => {
    ...
    setError('');
    try {
      const response = await fetch('http://localhost:9024/v1/order_alert/all-order', {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${user?.token}`,
        },
    ...
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center space-x-2">
          ...
          {/* All Orders - Only for Admin & Super Admin */}
          {(user?.role === 1 || user?.role === 2) && (
            <Button
              onClick={fetchAllOrders}
              disabled={loading}
              className="flex items-center space-x-2 bg-gradient-to-r from-pink-500 to-purple-600 text-white hover:from-pink-600 hover:to-purple-700"
            >
              <Package className="w-4 h-4" />
                ...
                {order.is_canceled && (
                  <div className="mt-4 p-3 bg-red-50 border border-red-200 rounded-lg">
                    ...
                    <p className="text-red-600 text-sm">
                      This order has been canceled
                      {order.canceled_at &&
                        ` on ${new Date(order.canceled_at).toLocaleDateString('en-IN')}`}
                    </p>
                  </div>
                )}

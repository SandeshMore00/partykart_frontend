...
export default function AdminDashboard() {
  ...
  const fetchAllSubcategories = async () => {
    try {
      const response = await fetch(config.CATEGORY_SUBCATEGORY(categoryId), {
        headers: { 'Authorization': `Bearer ${user?.token}` }
      });
      if (response.ok) {
        const result = await response.json();
    ...
    try {
      ...
      const fetchSubcategories = async () => {
        try {
          ...
          const response = await fetch(config.CATEGORY_SUBCATEGORY(categoryId), { headers });
          if (response.ok) {
            const result = await response.json();
            if (result.status === 1 && result.data) {
              setSubcategories(result.data);
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      console.log('Making request to:', `${config.ORDER_ALERT_SERVICE_URL}/v1/order_alert/`);
      console.log('Headers:', headers);

      // const response = await fetch('http://localhost:9024/v1/order_alert/', {
      const response = await fetch(`${config.ORDER_ALERT_SERVICE_URL}/v1/order_alert/`, {
        method: 'GET',
        headers
      });
  ...
  const handleUpdateProduct = async () => {
    ...
    try {
      const response = await fetch(`${config.PRODUCTS_SERVICE_URL}/v1/products/update/${editingProduct.product_id}`, {
        method: 'PATCH',
        headers: {
          'Authorization': `Bearer ${user?.token}`
        },
  ...
  const handleCreateCategory = async () => {
    ...
    try {
      ...
      const response = await fetch(config.CATEGORY_SERVICE_URL + '/create', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${user?.token}`
        },
  ...
  const handleDeleteCategory = async (categoryId: number) => {
    ...
    setCategorySuccess('');
    try {
      const response = await fetch(config.CATEGORY_SERVICE_URL + `/delete/${categoryId}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${user?.token}`
        }
  ...
  const handleUpdateCategory = async () => {
    ...
    try {
      ...
      const formData = new FormData();
      formData.append('category_name', categoryForm.category_name);
      const response = await fetch(config.CATEGORY_SERVICE_URL + `/update/${editingCategory?.category_id}`, {
        method: 'PATCH',
        headers: {
          'Authorization': `Bearer ${user?.token}`
        },
  ...
  const handleCreateSubcategory = async () => {
    ...
    try {
      const headers = { 'Authorization': `Bearer ${user?.token}` };
      const subcategoriesResponse = await fetch(config.CATEGORY_SERVICE_URL + '/sub-category/', { headers });
  ...
  const handleDeleteSubcategory = async (subCategoryId: number) => {
    ...
    try {
      const response = await fetch(config.CATEGORY_SERVICE_URL + `/sub-category/delete/${subCategoryId}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${user?.token}`
        }
  ...
  const handleUpdateSubcategory = async () => {
    ...
    try {
      ...
      const subcategoriesResponse = await fetch(config.CATEGORY_SUBCATEGORY(categoryId), { headers });
      let latestSubcategories = subcategories;
      if (subcategoriesResponse.ok) {
        const subcategoriesResult = await subcategoriesResponse.json();
        if (subcategoriesResult.status === 1 && subcategoriesResult.data) {
      ...
      formData.append('sub_category_name', subcategoryForm.sub_category_name);
      formData.append('category_id', subcategoryForm.category_id);
      const response = await fetch(config.CATEGORY_SERVICE_URL + `/sub-category/update/${editingSubcategory?.sub_category_id}`, {
        method: 'PATCH',
        headers: {
          'Authorization': `Bearer ${user?.token}`
        },

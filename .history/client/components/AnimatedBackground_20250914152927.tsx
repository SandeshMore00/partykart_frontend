...
export default function LogoBackground() {
  ...
  useEffect(() => {
    ...
    setStars(Array.from({ length: 50 }, (_, i) => createStar(i)));
  }, [dimensions]);

  // Animate stars (flicker effect)
  useEffect(() => {
    const interval = setInterval(() => {
      setStars((prev) =>
        prev.map((star) => {
          let newOpacity = star.opacity + (Math.random() > 0.5 ? star.flickerSpeed : -star.flickerSpeed);

...
const DashboardCarousel: React.FC = () => {
  ...
  console.log('DashboardCarousel user:', user);
  console.log('DashboardCarousel isAdmin:', typeof isAdmin === 'function' ? isAdmin() : isAdmin);
  ...
  if (loading) {
    return (
      {/* DEBUG: Show user and role info for troubleshooting */}
      {process.env.NODE_ENV !== 'production' && (
        <div className="absolute top-2 left-2 bg-white/80 text-xs text-gray-700 p-2 rounded shadow z-50">
          <div>User: {user ? user.phone_no : 'None'}</div>
          <div>Role ID: {user ? user.role_id : 'None'}</div>
          <div>isAdmin: {typeof isAdmin === 'function' ? isAdmin() ? 'true' : 'false' : String(isAdmin)}</div>
        </div>
      )}

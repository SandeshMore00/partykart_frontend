...
//       const response = await fetch(config.FETCH_CATEGORY, { headers });
//       if (response.ok) {
...
export default function CategoryBar() {
  ...
  const [dropdownHovered, setDropdownHovered] = useState(false);
  const [categoryHovered, setCategoryHovered] = useState(false);
  const [isMobile, setIsMobile] = useState<boolean>(false); // 👈 track mobile
  ...
  const fetchCategories = async () => {
    try {
      ...
      if (response.ok) {
        const result = await response.json();
        if (result.status === 1 && result.data) setCategories(result.data);
      }
    } catch (error) {
      console.error('Error fetching categories:', error);
    } finally {
  ...
  const fetchSubcategories = async (categoryId: number) => {
    try {
      setSubcategories([]);
      const headers: Record<string, string> = { 'Content-Type': 'application/json' };
      const response = await fetch(config.CATEGORY_DETAILS(categoryId), { method: 'GET', headers });
      if (response.ok) {
        const result = await response.json();
        setSubcategories(result.status === 1 && Array.isArray(result.data) ? result.data : []);
      } else {
        setSubcategories([]);
      }
    } catch (error) {
  ...
  // Click works on both, but required on mobile
  const handleCategoryClick = (categoryId: number) => {
  ...
  if (loading) {
    return (
      <div className="bg-white border-b border-gray-100 w-full z-30 relative top-0">
        <div className="container mx-auto px-4 py-3">
          <div className="flex items-center justify-center">
            <div className="animate-pulse text-gray-400">Loading categories...</div>
          </div>
  ...
  return (
    <div className="bg-white border-b border-gray-100 shadow-sm w-full z-30 relative top-0 mt-[128px]">
      <div className="container mx-auto px-2 sm:px-4">
        <div className="flex flex-wrap items-center py-2 sm:py-3 gap-1 sm:gap-2 md:gap-4 lg:gap-6">
          {categories.map((category) => (
            <div
              ...
              onMouseLeave={() => !isMobile && setCategoryHovered(false)}
            >
              {/* Category Button */}
              <button
                className={`flex items-center space-x-1 sm:space-x-2 px-2 sm:px-3 py-1.5 sm:py-2 
                  text-xs sm:text-sm md:text-base text-gray-700 hover:text-pink-600 
                  hover:bg-pink-50 rounded-lg transition-all duration-200 whitespace-nowrap group 
        ...
        {((!isMobile && hoveredCategory && (categoryHovered || dropdownHovered)) ||
          (isMobile && activeCategory)) && (
          <div
            id="category-dropdown"
            className="w-full bg-white shadow-xl rounded-lg border border-gray-200 mt-2 animate-in slide-in-from-top duration-200"
            onMouseEnter={() => !isMobile && setDropdownHovered(true)}
            onMouseLeave={() => {
              if (!isMobile) {

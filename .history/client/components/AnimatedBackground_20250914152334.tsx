...
export default function LogoBackground() {
  ...
  useEffect(() => {
    ...
    const createLogo = (id: number): LogoParticle => ({
      ...
      speedY: -Math.random() * 0.7 - 0.3, // upward float
      rotation: Math.random() * 360,
    ...
    const interval = setInterval(() => {
      setLogos((prev) =>
        prev.map((logo) => {
          ...
          // Respawn when logo goes above the screen
          if (newY < -150) {
            return {
              ...logo,
              x: Math.random() * dimensions.width,
  ...
  return (
    <div className="fixed inset-0 overflow-hidden pointer-events-none z-0">
      {logos.map((logo) => {
        // Fade based on vertical position (0 at top, 1 at bottom)
        const progress =
          1 - Math.max(0, Math.min(1, logo.y / dimensions.height));
        const opacity = Math.max(0, 0.6 - progress * 0.6); // start faded, vanish at top
